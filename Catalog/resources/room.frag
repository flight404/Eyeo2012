...
//	}
	...
	float sBorder		= 1.0 - pow( sin( gl_TexCoord[0].s * 3.14159 ), 0.004 );
	float tBorder		= 1.0 - pow( sin( gl_TexCoord[0].t * 3.14159 ), 0.004 );
	
	vec3 litRoomColor	= vec3( aoLight + ( ceiling + ceilingGlow ) * lightPower );
	vec3 darkRoomColor	= vec3( aoDark + max( sBorder, tBorder ) * vec3( 2.0, 3.0, 4.0 ) * lightPower );
