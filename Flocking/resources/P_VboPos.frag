...
vec3 PointLight( vec3 vertex, vec3 normal, vec3 eyeDirN, vec3 lightCenter, float lightRadius, vec3 color, float cutoff )
{
    ...
    return color * diff * 10.0 * attenuation + spec ;
}
...
void main()
{
	...
	vec3 blueish		= vec3( 0.035, 0.2, 0.15 );
	vec3 litRoomColor	= vec3( vNormal.y * 0.5 + 0.5 );
	vec3 darkRoomColor	= baitLighting;
	
	gl_FragColor.rgb	= mix( litRoomColor, darkRoomColor, power ) + ( vNormal * 0.5 + 0.5 ) * 0.2;
	gl_FragColor.a		= 1.0;
}
