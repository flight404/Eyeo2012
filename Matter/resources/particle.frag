...
//		float noise			= rand( gl_TexCoord[0].st + vec2( time, time ) );
		...
		vec3 litRoomColor	= c * vec3( fakeAo * shine + centerGlow * fakeAo - topShadow + rimLight * 2.0 ) + envColor * ppEyeFres * 0.7 + envColor * ppEyeDiff * 0.4 + c * 0.3;
