...
const float MAX_TIMEMULTI	= 80.0f;
...
{
	return Vec3f( Rand::randFloat( -mDims.x, mDims.x ),
 				  Rand::randFloat( -mDims.y, mDims.y ),
 				  Rand::randFloat( -mDims.z, mDims.z ) ) * 0.85f;
}
...
{
	return Vec3f( -mDims.x, 0.0f, 0.0f );
}

Vec3f Room::getRightWallPos()
{
	return Vec3f( mDims.x, 0.0f, 0.0f );
}
